		...
		final def = macro class $name
			{
				...
				inline function get_length()
					return Std.int(this.length / FIELDS);
					
				public inline function new()
				{
					$e
					{
						isVector ? macro this = new openfl.Vector() : macro this = []
					}
				}
				...
				public inline function resize(length:Int)
				{
					$e
					{
						isVector ? macro this.length = length * FIELDS : macro this.resize(length * FIELDS)
					}
				}
